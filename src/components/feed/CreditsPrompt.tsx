    ...
    <div className="bg-card border border-border rounded-xl p-6 text-center shadow-soft">
      ...
      <div className="space-y-2 mb-4 text-left max-w-xs mx-auto">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>Post a thread → <span className="text-foreground font-bold">+10 credits</span></span>
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <PenSquare className="h-3.5 w-3.5 text-primary" />
      ...
      {profile && <p className="text-[11px] text-muted-foreground mb-3">Your credits: <span className="font-bold text-foreground">{profile.credits}</span></p>}
      <Button onClick={() => navigate("/submit")} className="rounded-full font-bold text-xs">Create a Post to Unlock</Button>
    </div>
  );
}
