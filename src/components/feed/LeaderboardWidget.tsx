  ...
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-accent/30">
        ...
        <h3 className="font-bold text-xs text-foreground uppercase tracking-wider">Top Contributors</h3>
      </div>
      <div className="p-2">
        {activeLeaders.slice(0, 5).map((leader, i) => (
          <div key={leader.user_id} className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-accent/40 transition-colors">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-bold text-foreground">{leader.credits}</span>
            </div>
          </div>
        ))}
      </div>
