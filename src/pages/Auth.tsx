...
export default function Auth() {
  ...
  const stepIcon = () => {
    switch (step) {
      ...
      case "signup-email":
      case "forgot-otp": return <Mail className="h-5 w-5 text-primary" />;
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-1 items-center justify-center bg-gradient-to-br from-primary/10 via-background to-background relative overflow-hidden">
        <div className="relative z-10 text-center px-12">
          <img src={dmLogo} alt="Digital Mitra" className="h-16 w-16 rounded-2xl mx-auto mb-6 shadow-glow" />
        ...
        <div className="absolute -top-32 -right-32 h-96 w-96 rounded-full bg-primary/3 blur-3xl" />
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-sm">
          ...
          {/* Step progress (signup/forgot flows) */}
          {stepNumber > 0 && (
              ...
              {[1, 2, 3].map((s) => (
                <div
                  ...
                  className={cn(
                    ...
                    s === stepNumber ? "w-6 bg-primary" : s < stepNumber ? "w-1.5 bg-primary/50" : "w-1.5 bg-border"
                  )}
                />
              ))}
            </div>
            ...
            >
              ...
              {step === "login" && (
                <form onSubmit={(e) => { e.preventDefault(); loginWithPassword(); }} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="email" type="email" placeholder="yourname@iimb.ac.in" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-lg bg-secondary/40 border-border/30" required />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="password" className="text-xs">Password</Label>
                    <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="rounded-lg bg-secondary/40 border-border/30" required />
                  </div>
                  <Button type="submit" className="w-full rounded-lg font-semibold" disabled={loading}>
                    {loading ? "Signing in…" : "Sign In"}
                  </Button>
                  <div className="flex items-center justify-between text-sm pt-1">
                    <button type="button" className="text-primary hover:underline font-medium text-xs" onClick={() => { setOtpMode("signup"); setPassword(""); setStep("signup-email"); }}>
                      New here? Sign up
                    </button>
                    <button type="button" className="text-muted-foreground hover:text-foreground text-xs" onClick={() => { setOtpMode("forgot"); setPassword(""); setStep("forgot-otp"); }}>
                      Forgot password?
                    </button>
                  </div>
                </form>
              ...
              {(step === "signup-email" || step === "forgot-otp") && (
                <form onSubmit={(e) => { e.preventDefault(); sendOtp(); }} className="space-y-4">
                  ...
                  <button type="button" className="w-full text-xs text-muted-foreground hover:text-foreground flex items-center justify-center gap-1 pt-1" onClick={goBack}>
              ...
              {step === "verify-otp" && (
                <div className="space-y-4">
                  ...
                  <Button className="w-full rounded-lg font-semibold" disabled={loading || otp.length !== 6} onClick={verifyOtp}>
                    {loading ? "Verifying…" : "Verify Code"}
                  </Button>
                  <div className="flex items-center justify-between text-xs">
                    <button type="button" className="text-muted-foreground hover:text-foreground flex items-center gap-1" onClick={goBack}>
                      <ArrowLeft className="h-3 w-3" /> Back
                    </button>
                    <button type="button" className="text-primary font-medium hover:underline disabled:opacity-50" disabled={cooldown > 0} onClick={sendOtp}>
              ...
              {step === "set-password" && (
                <form onSubmit={(e) => { e.preventDefault(); setUserPassword(); }} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="new-password" type="password" placeholder="At least 6 characters" value={password} onChange={(e) => setPassword(e.target.value)} className="rounded-lg bg-secondary/40 border-border/30" required minLength={6} />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="confirm-password" className="text-xs">Confirm Password</Label>
                    <Input id="confirm-password" type="password" placeholder="Re-enter password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="rounded-lg bg-secondary/40 border-border/30" required minLength={6} />
                  </div>
                  <Button type="submit" className="w-full rounded-lg font-semibold" disabled={loading}>
                    {loading ? "Setting…" : otpMode === "signup" ? "Create Account" : "Reset Password"}
                  </Button>
                  <button type="button" className="w-full text-xs text-muted-foreground hover:text-foreground flex items-center justify-center gap-1" onClick={goBack}>
                    <ArrowLeft className="h-3 w-3" /> Back
                  </button>
                </form>
              )}
